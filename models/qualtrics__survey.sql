...
calc_medians as (
    ...
    from (
        select 
            survey_id, 
            source_relation,
            {{ fivetran_utils.percentile(percentile_field='cast(duration_in_seconds as ' ~ dbt.type_float() ~ ')', partition_field='survey_id,source_relation', percent='0.5') }} as median_response_duration_in_seconds,
            {{ fivetran_utils.percentile(percentile_field='cast(progress as ' ~ dbt.type_float() ~ ')', partition_field='survey_id,source_relation', percent='0.5') }} as median_survey_progress_pct
